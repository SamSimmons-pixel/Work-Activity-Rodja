<div class="relative" x-data="{ open: false }" @click.outside="open = false" @keydown.escape.window="open = false">
    ...
    >
        ...
        <span
            ...
            class="{{ $unreadCount > 0 ? '' : 'hidden' }} absolute -top-0.5 -right-0.5 flex items-center justify-center min-w-[1.25rem] h-5 px-1 text-3xs font-black text-white bg-rose-600 rounded-full ring-2 ring-white shadow-xs animate-pulse"
    ...
    >
        ...
        <div class="px-4 py-3.5 border-b border-slate-100 bg-slate-50/80 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-2">
                ...
                <span id="notification-header-badge" class="{{ $unreadCount > 0 ? '' : 'hidden' }} px-2 py-0.5 rounded-full text-3xs font-extrabold bg-rose-100 text-rose-700">
                    <span id="notification-header-count">{{ $unreadCount }}</span> Baru
                </span>
            </div>
        ...
        <div class="px-3 pt-2 pb-1 bg-white border-b border-slate-100 flex items-center gap-1 shrink-0">
            ...
            >
                Belum Dibaca (<span id="notification-filter-count">{{ $unreadCount }}</span>)
            </button>
        </div>

        <!-- Notifications Scrollable List Area -->
        <div id="notification-items-list" class="overflow-y-auto flex-1 divide-y divide-slate-100">
            ...
            @empty
                ...
                <div id="notification-empty-state" class="p-8 text-center space-y-3">
