<div
    ...
    x-data="notificationMenu({
        ...
        initialNotifications: {{ Js::from($notifications->map(fn($notif) => [
            'id' => (string) $notif->id,
            'read_at' => $notif->read_at,
            'title' => $notif->data['title'] ?? 'Pemberitahuan Sistem',
            'message' => $notif->data['message'] ?? '-',
            'sender_name' => $notif->data['sender_name'] ?? null,
            'sender_role' => $notif->data['sender_role'] ?? null,
            'type' => $notif->data['type'] ?? 'general',
            'url' => $notif->data['url'] ?? null,
            'time_ago' => $notif->created_at->diffForHumans(null, true),
        ])) }}
    })"
...
>
    ...
    >
        ...
        <span
            ...
            class="absolute -top-0.5 -right-0.5 flex items-center justify-center min-w-[1.25rem] h-5 px-1 text-3xs font-black text-white bg-rose-600 rounded-full ring-2 ring-white shadow-xs animate-pulse"
        ></span>
    </button>
    ...
    >
        ...
        <div class="px-4 py-3.5 border-b border-slate-100 bg-slate-50/80 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-2">
                ...
                <span
                    ...
                    class="px-2 py-0.5 rounded-full text-3xs font-extrabold bg-rose-100 text-rose-700"
                ></span>
            </div>
        ...
        <div class="px-3 pt-2 pb-1 bg-white border-b border-slate-100 flex items-center gap-1 shrink-0">
            <button
                @click="filter = 'all'"
                type="button"
                :class="filter === 'all' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'"
                class="px-3 py-1 text-2xs font-bold rounded-lg transition cursor-pointer"
            >
                Semua
            </button>
            <button
                @click="filter = 'unread'"
                type="button"
                :class="filter === 'unread' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'"
                class="px-3 py-1 text-2xs font-bold rounded-lg transition cursor-pointer"
            >
                <span x-text="`Belum Dibaca (${unreadCount})`"></span>
            </button>
        </div>
        ...
        <div class="overflow-y-auto flex-1 divide-y divide-slate-100">
            <template x-for="notif in filteredNotifications" :key="notif.id">
                ...
                <div
                    :class="!notif.read_at ? 'bg-indigo-50/50 hover:bg-indigo-50/80 border-l-4 border-indigo-600' : 'bg-white hover:bg-slate-50 border-l-4 border-transparent'"
                    class="group relative flex items-start gap-3 p-3.5 transition cursor-pointer"
                >
                    <!-- Type Icon Badge -->
                    <div class="shrink-0 mt-0.5">
                        ...
                        <template x-if="notif.type === 'verified'">
                        ...
                        <template x-if="notif.type === 'review'">
                        ...
                        <template x-if="notif.type !== 'comment' && notif.type !== 'verified' && notif.type !== 'review'">
                    ...
                    >
                        <div class="flex items-center justify-between gap-1">
                            <h4 :class="!notif.read_at ? 'font-black text-slate-900' : 'font-semibold text-slate-700'" class="text-xs truncate" x-text="notif.title || 'Pemberitahuan Sistem'"></h4>
                            <span class="text-3xs text-slate-400 shrink-0" x-text="notif.time_ago"></span>
                        </div>

                        <p :class="!notif.read_at ? 'text-slate-800 font-medium' : 'text-slate-500'" class="text-2xs line-clamp-2 mt-0.5 leading-relaxed" x-text="notif.message"></p>

                        <template x-if="notif.sender_name">
                            <div class="flex items-center gap-1.5 mt-1 text-3xs text-slate-400">
                                <span class="font-bold text-slate-600" x-text="notif.sender_name"></span>
                                <template x-if="notif.sender_role">
                                    <span>&bull; <span x-text="notif.sender_role"></span></span>
                                </template>
                            </div>
                        </template>
                    </div>
            ...
            <div x-show="filteredNotifications.length === 0" class="p-8 text-center space-y-3">
                ...
                <div>
                    ...
                    <p class="text-3xs text-slate-400 mt-0.5" x-text="filter === 'unread' ? 'Semua notifikasi telah Anda baca.' : 'Aktivitas, komentar, dan verifikasi baru akan muncul di sini.'"></p>
                </div>
            </div>
        </div>
    </div>
